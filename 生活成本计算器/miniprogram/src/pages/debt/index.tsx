...
import { computeLoanApr, computeAffordableDebt, buildLoanAprPrompt, buildAffordableDebtPrompt } from '../../core';
...
export default function DebtPage() {
  ...
  const onApr = () => { setApr(computeLoanApr(Number(principal) || 0, Number(monthly) || 0, Number(periods) || 0)); setPrompt(''); };
  const onAff = () => { setAff(computeAffordableDebt(Number(surplus) || 0, (Number(aprPct) || 0) / 100, Number(periods2) || 0)); setPrompt(''); };
  const onAskApr = () => setPrompt(buildLoanAprPrompt(Number(principal) || 0, Number(monthly) || 0, Number(periods) || 0));
  const onAskAff = () => setPrompt(buildAffordableDebtPrompt(Number(surplus) || 0, Number(aprPct) || 0, Number(periods2) || 0));
  ...
  return (
    <View className="page">
      ...
      <View className="card calc">
        ...
        {apr && !apr.error && (
          <View className="result-box">
            <View className="big-line">真实年化<Text className={`rate rate-${levelClass(apr.level)}`}>{(apr.annual_irr * 100).toFixed(1)}%</Text><Text className={`rate-tag tag-${levelClass(apr.level)}`}>{apr.level}</Text></View>
            <View className="info-row"><Text className="info-label">总利息</Text><Text className="info-val">{fmtNum(apr.interest)} 元（占本金 {(apr.interest_ratio * 100).toFixed(0)}%）</Text></View>
            <View className="note">{apr.note}</View>
            <Button className="btn-ask" onClick={onAskApr}>问 AI：这贷款正常吗</Button>
          </View>
        )}
      </View>
      ...
      <View className="card calc">
        ...
        {aff && !aff.error && (
          <View className="result-box">
            <View className="info-row"><Text className="info-label">最多能借（激进）</Text><Text className="info-val strong">{fmtNum(aff.max_principal)} 元</Text></View>
            <View className="info-row"><Text className="info-label">稳妥档（保守）</Text><Text className="info-val">{fmtNum(aff.safe_principal)} 元</Text></View>
            <View className="note">{aff.note}</View>
            <Button className="btn-ask" onClick={onAskAff}>问 AI：我借这笔安全吗</Button>
          </View>
        )}
      </View>
